...
export default {
    ...
    ryzumi: async function ryzumi(buffer) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const { data: ryzumi } = await axios.post('https://api-02.ryzumi.vip/api/uploader/ryzencdn', data, { headers: { ...data.getHeaders(), referrer: "https://api.ryzumi.vip/" } })
                resolve(ryzumi);
            } catch (e) {
                reject({
                    msg: 'Gomene Error Tourl'
    ...
    cloudku: async function(buffer) {
        return new Promise(async (resolve, reject) => {
            try {
              const { filename } = await file(buffer);
              return cloudku.uploadFile(buffer, filename);
            } catch (err) {
                reject({
                    msg: 'Gomene Error Tourl'
                });
